...
export async function listAllTags(request: ListTagsRequest): Promise<UseCaseResult<readonly Data[]>> {
  const authorization = await authorizeFind({ guard: TAGS_ALL, identity: request.identity, select: request.select, context: request.context });
...
export async function createTag(request: CreateTagRequest): Promise<UseCaseResult<Tag>> {
  const authorization = await authorizeWrite({ guard: TAGS_ALL, identity: request.identity, method: 'create', data: request.data, context: request.context });
  if (!authorization.isAllowed) return deny(authorization.errors);
...
export async function removeTag(request: RemoveTagRequest): Promise<UseCaseResult<undefined>> {
  const authorization = await authorizeWrite({ guard: TAGS_ALL, identity: request.identity, method: 'remove', data: { id: request.id }, context: request.context });
  if (!authorization.isAllowed) return deny(authorization.errors);
