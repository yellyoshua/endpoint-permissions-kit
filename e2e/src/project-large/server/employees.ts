...
export async function listAllEmployees(request: ListEmployeesRequest): Promise<UseCaseResult<readonly Data[]>> {
  const authorization = await authorizeFind({ guard: EMPLOYEES_ALL, identity: request.identity, select: request.select, context: request.context });
...
export async function createEmployee(request: EmployeeWriteRequest): Promise<UseCaseResult<Employee>> {
  const authorization = await authorizeWrite({ guard: EMPLOYEES_ALL, identity: request.identity, method: 'create', data: request.data, context: request.context });
  if (!authorization.isAllowed) return deny(authorization.errors);
...
export async function updateEmployee(request: EmployeeUpdateRequest): Promise<UseCaseResult<Employee>> {
  const authorization = await authorizeWrite({ guard: EMPLOYEES_ALL, identity: request.identity, method: 'update', data: request.data, context: request.context });
  if (!authorization.isAllowed) return deny(authorization.errors);
...
export async function removeEmployee(request: EmployeeRemoveRequest): Promise<UseCaseResult<undefined>> {
  ...
  const authorization = await authorizeWrite({ guard: EMPLOYEES_ALL, identity: request.identity, method: 'remove', data: { id: request.id }, context: request.context });
  if (!authorization.isAllowed) return deny(authorization.errors);
