...
export async function listAllInvoices(request: ListInvoicesRequest): Promise<UseCaseResult<readonly Data[]>> {
  const authorization = await authorizeFind({ guard: INVOICES_ALL, identity: request.identity, select: request.select });
...
export async function createInvoice(request: InvoiceWriteRequest): Promise<UseCaseResult<Invoice>> {
  const authorization = await authorizeWrite({ guard: INVOICES_ALL, identity: request.identity, method: 'create', data: request.data });
  if (!authorization.isAllowed) return deny(authorization.errors);
...
export async function updateInvoice(request: InvoiceUpdateRequest): Promise<UseCaseResult<Invoice>> {
  ...
  const authorization = await authorizeWrite({ guard: INVOICES_ALL, identity: request.identity, method: 'update', data: request.data });
  if (!authorization.isAllowed) return deny(authorization.errors);
...
export async function removeInvoice(request: InvoiceRemoveRequest): Promise<UseCaseResult<undefined>> {
  ...
  const authorization = await authorizeWrite({ guard: INVOICES_ALL, identity: request.identity, method: 'remove', data: { id: request.id } });
  if (!authorization.isAllowed) return deny(authorization.errors);
